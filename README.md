...
Optional TypePHP native accelerator for [eloquage/score](https://github.com/eloquage/score).
...
```sh
pie install eloquage/score-ext
```
...
```
php_eloquage_score-X.Y.Z_php8.5-x86_64-Linux-glibc-N-debug-NTS.tar.gz
```

The archive contains `eloquage_score.so`. Windows, macOS, ZTS, and non-8.5 binaries are not currently published.
